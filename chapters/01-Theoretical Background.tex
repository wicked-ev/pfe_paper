...
\begin{figure}[htbp]
	...
	\begin{minipage}[b]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figures/2st_right.png} % change to your actual filename
		\caption{(a) Two-stage Faster R-CNN}
		\label{fig:faster_rcnn}
	\end{minipage}
	\hfill % Maximizes horizontal separation within the normal margins
	% Right Subfigure (b)
	\begin{minipage}[b]{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figures/2st_left.png} % change to your actual filename
		\caption{(b) One-stage RetinaNet}
		\label{fig:retinanet}
	\end{minipage}
	
	\vskip 10pt
	\caption{Figure 2.1: (a) Two-stage Faster R-CNN with an intermediary region proposal generator. (b) One-stage RetinaNet, which consists of a dense detection pipeline.}
...
\end{enumerate}
 ...
 \begin{figure}[H]
 	\centering
 	% Scaled to \textwidth to guarantee full legibility of layer annotations and kernel operations
 	\includegraphics[width=0.8\textwidth]{figures/convolutional-neural-networks-cnns.png} 
 	\caption{The Architectural pipeline of a deep CNN. It shows the sequential feature extraction through alternating convolution and pooling layers followed by the flattened vector feeding into a fully connected layer and producing probabilistic softmax classification.}
 	\label{fig:cnn_architecture_pipeline}
 \end{figure}
...
\begin{figure}[H]
	\centering
	\begin{tikzpicture}[font=\sffamily\bfseries, align=center]
		
		% Outer Circle: Artificial Intelligence
		\draw[thick, cyan, fill=cyan!2] (0,1.8) circle (3.2cm);
		\node[text=black!80] at (0,4.0) {Artificial\\Intelligence};
		
		% Middle Circle: Machine Learning
		\draw[thick, orange!80, fill=orange!5] (0,0.8) circle (2.2cm);
		\node[text=black!80] at (0,1.8) {Machine\\Learning};
		
		% Inner Circle: Deep Learning
		\draw[thick, green!50!black, fill=green!5] (0,0) circle (1.2cm);
		\node[text=black!80] at (0,0) {Deep\\Learning};
		
	\end{tikzpicture}
	\caption{Taxonomical taxonomy mapping the nested subsets of modern computational intelligence paradigms.}
	\label{fig:ai_ml_dl_taxonomy}
\end{figure}
...
\begin{table}[H]
	...
	% Forces the entire table to take up only 85% of the page width
	% The !{\color{white}\vrule} syntax creates clean, modern separators instead of harsh black borders
	\begin{tabularx}{0.85\textwidth}{
			>{\raggedright\arraybackslash}X 
			!{\color{white}\vrule} c 
			!{\color{white}\vrule} c 
			!{\color{white}\vrule} >{\raggedright\arraybackslash}X}
		\toprule
		\rowcolor[HTML]{2C3E50} 
		\textcolor{white}{\textbf{Model}} & 
		\textcolor{white}{\textbf{Params (M)}} & 
		\textcolor{white}{\textbf{mAP (COCO)}} & 
		\textcolor{white}{\textbf{Architecture}} \\
		\midrule
		
		YOLOv8n        & 3.2  & 37.3 & CNN \\
		
		\rowcolor[HTML]{F2F4F4} 
		RT-DETR-R18    & 20.0 & 46.5 & Transformer \\
		
		RF-DETR Nano   & --   & --   & ViT + DETR \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\begin{figure}[H]
	\centering
	% Scaled to full text width to ensure text legends and directional keys are crisp and legible
	\includegraphics[width=\textwidth]{figures/ddd.png} 
	\caption{Architecture of the multi-scale Deformable DETR network pipeline. Left: Extraction of image feature maps across variable spatial scales. Center: The Encoder processing stack utilizing multi-scale deformable self-attention layers. Right: The Decoder structure routing object queries through multi-scale cross-attention and standard transformer self-attention mechanisms to output localized bounding box predictions.}
	\label{fig:deformable_detr_architecture}
\end{figure}
...
or converted further into a hardware-specific format.
 ...
 \begin{figure}[H]
 	\centering
 	% Scaled to \textwidth to guarantee absolute legibility of framework and hardware text labels
 	\includegraphics[width=\textwidth]{figures/0_o5doPyWdduatUKtX.png} 
 	\caption{The open and interoperable ecosystem of the Open Neural Network Exchange (ONNX) framework. ONNX decouples the training infrastructure (left) from heterogeneous runtime deployment targets and specialized hardware accelerators (right), standardizing graph compilation across diverse edge and cloud environments.}
 	\label{fig:onnx_interoperability_ecosystem}
 \end{figure}
...
\begin{table}[H]
	...
	% Scaled to 0.85\textwidth to match your object detection table perfectly
	% Uses white vertical rule separators for crisp column lanes
	\begin{tabularx}{0.85\textwidth}{
			>{\raggedright\arraybackslash}X 
			!{\color{white}\vrule} c 
			!{\color{white}\vrule} c 
		...
		\textcolor{white}{\textbf{Training Required}} & 
